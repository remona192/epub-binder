...
class MergeWorker(QThread):
    ...
    def __init__(self, epub_files, output_path, title, add_toc, toc_titles,
        ...
        self.extra_front_covers    = list(extra_front_covers or [])
            ...
            for idx, epub_path in enumerate(self.epub_files):
                ...
                if idx == 0:
                    ...
                    if cover_data:
                        ...
                        for extra_idx, extra in enumerate(self.extra_front_covers, start=1):
                            try:
                                extra_data, extra_ext, _extra_label = extra
                                if not extra_data:
                                    continue
                                extra_ext = extra_ext if str(extra_ext).startswith(".") else f".{extra_ext}"
                                extra_ext = str(extra_ext or ".jpg").lower()
                                extra_img_id = f"extra-cover-image-{extra_idx}"
                                extra_html_id = f"extra-cover-html-{extra_idx}"
                                extra_fname = f"extra_cover_{extra_idx}{extra_ext}"
                                extra_dst = os.path.join(images_dir, extra_fname)
                                with open(extra_dst, "wb") as _ef:
                                    _ef.write(extra_data)
                                extra_mt = "image/jpeg" if extra_ext in (".jpg", ".jpeg") else "image/png"
                                all_manifest[extra_img_id] = (f"OEBPS/Images/{extra_fname}", extra_mt)
                                extra_xhtml = (
                                    "<?xml version='1.0' encoding='utf-8'?>\n"
                                    '<html xmlns="http://www.w3.org/1999/xhtml">\n'
                                    "<head>\n"
                                    f"<title>표지 {extra_idx + 1}</title>\n"
                                    "<style>\n"
                                    "    @page { padding:0; margin:0; }\n"
                                    "    body { text-align:center; padding:0; margin:0; }\n"
                                    "    div { margin:0; padding:0; }\n"
                                    "</style>\n</head>\n<body>\n"
                                    f'<h2 style="display:none;">표지 {extra_idx + 1}</h2>\n'
                                    f'<div><img alt="cover {extra_idx + 1}" src="../Images/{extra_fname}"/></div>\n'
                                    "</body>\n</html>"
                                )
                                extra_xhtml_name = f"extra_cover_{extra_idx}.xhtml"
                                extra_xhtml_dst = os.path.join(text_base, extra_xhtml_name)
                                Path(extra_xhtml_dst).write_text(extra_xhtml, encoding="utf-8")
                                all_manifest[extra_html_id] = (
                                    f"OEBPS/Text/{extra_xhtml_name}", "application/xhtml+xml")
                                all_spine.insert(
                                    extra_idx,
                                    (extra_html_id, f"OEBPS/Text/{extra_xhtml_name}", -1, f"표지 {extra_idx + 1}", ""),
                                )
                            except Exception as ex:
                                self.log(f"⚠ 추가 표지 {extra_idx} 처리 실패: {ex}", "warn")
                    else:
