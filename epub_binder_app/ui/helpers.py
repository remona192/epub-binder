...
def _side_story_rank(value: str) -> int:
    text = str(value or "")
    if re.search(r'에필로그|후일담', text, re.IGNORECASE):
        return 1
    if re.search(r'특별\s*외전|특별외전|특\s*외|특외', text, re.IGNORECASE):
        return 3
    if re.search(r'외전|번외', text, re.IGNORECASE):
        return 2
    if re.search(r'특전', text, re.IGNORECASE):
        return 4
    return 0
...
def natural_sort_key(value: str):
    ...
    side_rank = _side_story_rank(text)
    is_extra = side_rank > 0
    ...
    if is_interview:
        return (series_name, 2, extra_num or number, part_num, 5, complete_flag, tag, text)
    if is_extra:
        return (series_name, 2, extra_num or number, part_num, side_rank, complete_flag, tag, text)
    return (series_name, 1, number, part_num, 0, complete_flag, tag, text)


def filename_natural_sort_key(value: str):
    return natural_sort_key(PathLikeName(value))
