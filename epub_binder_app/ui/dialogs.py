...
def _visual_signature(data: bytes):
    try:
...
def filter_visual_duplicate_candidates(candidates: list[dict]) -> list[dict]:
    filtered: list[dict] = []
    signatures = []
    for candidate in candidates:
        sig = _visual_signature(candidate.get("data") or b"")
        if sig is not None and any(_signature_distance(sig, other) <= 6 for other in signatures):
            continue
...
class CoverPickerDialog(QDialog):
    ...
    def __init__(self, parent, candidates: list,
        ...
        self._chosen_idx         = None      # 선택한 대표 표지 인덱스
        self._front_cover_keys   = []        # 추가 표지 선택 순서
        ...
        if len(self._all_epub_files) > 1 and not show_browse_toggle:
        ...
        if len(self._all_epub_files) > 1:
            self._candidates = self._collect_all_volume_candidates(False)
            self._hdr_label.setText(
                f"표지 후보 {len(self._candidates)}개 발견  ·  전체 표지 후보"
            )
    ...
    # 썸네일 렌더링
    def _front_cover_key(self, item: dict) -> str:
        vol_path = str(item.get('_vol_path') or self._current_vol_path or '')
        filename = str(item.get('filename') or '')
        digest = hashlib.sha1(item.get('data') or b'').hexdigest()
        return f"{vol_path}|{filename}|{digest}"

    def _collect_all_volume_candidates(self, include_all_images: bool):
        """권별 EPUB에서 표지 후보 또는 전체 이미지를 모아 한 화면에 보여줄 목록 생성."""
        items = []
        for vol_index, (vol_path, vol_label) in enumerate(self._all_epub_files):
            try:
                with open(vol_path, 'rb') as _fv:
                    eb = _fv.read()
                cands = extract_cover_candidates(eb, include_all_images=include_all_images)
            except Exception:
                continue
            deleted = self._deleted_by_vol.get(vol_path, set())
            for cand_index, cand in enumerate(cands):
                if cand.get('filename') in deleted:
                    continue
                item = dict(cand)
                item['_vol_index'] = vol_index
                item['_candidate_index'] = cand_index
                item['_vol_path'] = vol_path
                item['_vol_label'] = vol_label
                base_label = str(item.get('label') or Path(item.get('filename', '')).name).strip()
                item['label'] = f"{vol_index + 1:02d}. {vol_label} - {base_label}"
                items.append(item)
        return filter_visual_duplicate_candidates(items)

    def _on_front_cover_toggled(self, item: dict, checked: bool):
        key = self._front_cover_key(item)
        if checked:
            if key not in self._front_cover_keys:
                self._front_cover_keys.append(key)
        else:
            self._front_cover_keys = [k for k in self._front_cover_keys if k != key]
    ...
    def _render_thumbs(self, items: list):
        ...
        for i, c in enumerate(items):
            ...
            front_chk = QCheckBox("표지 추가")
            front_chk.setStyleSheet(
                f"QCheckBox{{color:{C['accent']};font-size:11px;background:transparent;font-weight:600;}}"
                f"QCheckBox::indicator{{width:13px;height:13px;"
                f"border:1.5px solid {C['accent']};border-radius:3px;background:{C['surface']};}}"
                f"QCheckBox::indicator:checked{{background:{C['accent']};border:1.5px solid {C['accent']};}}")
            front_chk.setToolTip(
                "대표 표지는 라디오 선택으로 기본 1개 들어갑니다.\n"
                "체크한 표지는 대표 표지 뒤에 추가로 순서대로 넣습니다.")
            front_key = self._front_cover_key(c)
            front_chk.setChecked(front_key in self._front_cover_keys)
            front_chk.toggled.connect(lambda checked, item=c: self._on_front_cover_toggled(item, checked))
            cl.addWidget(front_chk)
    ...
    def _on_show_all_toggled(self, checked: bool):
        if not self._epub_bytes and not self._all_epub_files:
            return
        self._show_all_mode = checked
        # 삭제 버튼 표시 토글
        if hasattr(self, '_b_delete'):
            self._b_delete.setVisible(checked)
        if self._all_epub_files:
            new_items = self._collect_all_volume_candidates(checked)
        else:
    ...
    def get_extra_front_covers(self):
        """병합 앞부분에 추가할 표지 목록을 선택 순서대로 반환."""
        by_key = {self._front_cover_key(item): item for item in self._candidates}
        selected_key = ""
        if self._chosen_idx is not None and self._chosen_idx < len(self._candidates):
            selected_key = self._front_cover_key(self._candidates[self._chosen_idx])
        chosen = []
        for key in self._front_cover_keys:
            if key == selected_key:
                continue
            item = by_key.get(key)
            if not item:
                continue
            chosen.append((item['data'], item['ext'], os.path.basename(item['filename'])))
        return chosen
