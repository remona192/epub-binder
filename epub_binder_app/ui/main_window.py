# -*- coding: utf-8 -*-
"""
Epub Binder v4.3.4  —  PyQt6 Edition
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QEvent, QRect
...
try:
    from epub_binder_core.txt_conversion import (
        normalize_txt_epub_job as _core_normalize_txt_epub_job,
        write_txt_epub_job as _core_write_txt_epub_job,
    )
    from epub_binder_core.name_cleanup import clean_series_title_author as _core_clean_series_title_author
except Exception:
    _core_normalize_txt_epub_job = None
    _core_write_txt_epub_job = None
...
def natural_sort_key(s):
    ...
    # 종류플래그: 0=일반권호, 1=외전/특별외전, 2=인터뷰/작가노트
    ...
    if is_interview: return (sn, 2, en or num, pn, 2, 0.5 if is_c else 0.0, tag, s)
    elif is_e:       return (sn, 2, en or num, pn, 1, 0.5 if is_c else 0.0, tag, s)
...
def extract_all_subheadings(raw_str: str):
    ...
    p_b_pat = re.compile(
        r'<(?P<wrap>p|div)\b[^>]*>\s*<(b|strong)\b[^>]*>(.*?)</\2>\s*</(?P=wrap)>',
        re.IGNORECASE | re.DOTALL)
    for m in p_b_pat.finditer(raw_str):
        ...
        if 1 < len(text) < 80 and m.start() not in seen_offsets:
...
# 챕터 마커 단독 (제목 없이 화수만): "1화", "제1화", "#001화", "프롤로그" 등
...
def extract_chapter_title(content_bytes: bytes):
    ...
    if combined and 2 < len(combined) < 80:
        return combined
    ...
    for m in _CENTER_BOLD_PAT.finditer(raw):
        ...
        text  = _normalize_title(text)
        text  = _safe_title(text)
        if _CHAP_NUM_TITLE.match(text) and 2 < len(text) < 80:
            return text
        # 권/화 번호 포함 텍스트도 유효한 챕터 제목으로 처리
        if re.search(r'\d+\s*[권화부]', text) and 2 < len(text) < 80:
            return text
        # #N 형식 챕터 제목 (예: #101 전쟁의 결과)
        if re.match(r'^[#＃]\s*\d+', text) and 2 < len(text) < 80:
            return text
...
def build_txt_epub(chapters, title: str, author: str,
                   cover_data: bytes = None, cover_ext: str = '.jpg') -> bytes:
    """챕터 리스트 → EPUB(zip) bytes 반환. (cover_data/ext: 표지 이미지 raw + 확장자)"""
    book_id = str(_uuid_mod.uuid4())
    css = (
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        if cover_data:
            ...
            cover_manifest = (
                ...
                f'<item id="cover-xhtml" href="cover.xhtml" '
        ...
        # 표지가 있으면 NCX 첫 항목으로 추가 (리디북스 등 엄격한 뷰어 호환)
        play_order = 1
        if cover_data:
            nav_points += (
                f"<navPoint id='cover' playOrder='{play_order}'>"
                f"<navLabel><text>표지</text></navLabel>"
                f"<content src='cover.xhtml'/></navPoint>")
            play_order += 1
        for i, (ch_t, ch_l) in enumerate(chapters):
            fname = f"ch_{i:04d}.xhtml"
            body = "".join(f"<p>{ln}</p>" for ln in ch_l) or "<p>&#160;</p>"
            safe_t = _html_mod.escape(ch_t or f"챕터 {i+1}")
            zf.writestr(f"OEBPS/{fname}",
                '<?xml version="1.0" encoding="utf-8"?>'
                '<html xmlns="http://www.w3.org/1999/xhtml"><head>'
                f'<title>{safe_t}</title>'
                '<link rel="stylesheet" type="text/css" href="style.css"/></head>'
                f'<body><h2>{safe_t}</h2>{body}</body></html>')
            manifest_items += (
                f'<item id="c{i}" href="{fname}" '
                f'media-type="application/xhtml+xml"/>')
            spine_items += f'<itemref idref="c{i}"/>'
            nav_points += (
                f"<navPoint id='n{i}' playOrder='{play_order}'>"
                f"<navLabel><text>{safe_t}</text></navLabel>"
        ...
        zf.writestr("OEBPS/content.opf",
            ...
            f'<dc:title>{_html_mod.escape(title)}</dc:title>'
            f'<dc:creator opf:role="aut">{_html_mod.escape(author)}</dc:creator>'
        ...
        zf.writestr("OEBPS/toc.ncx",
            ...
            f'<docTitle><text>{_html_mod.escape(title)}</text></docTitle>'
...
class MergeWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            for idx, epub_path in enumerate(self.epub_files):
                ...
                for iid in spine:
                    ...
                    if _core_merge_page_title_from_sources:
                        ...
                        page_title = self.page_title_overrides.get(override_key, "") if _has_title_override else (
                            extract_chapter_title(raw_str.encode('utf-8')) or ''
                        )
                    ...
                    if page_title:
                        page_title = _clean_chapter_display_title(page_title, self.title)
                        _page_title_key = _toc_duplicate_key(page_title)
                        if _page_title_key and _page_title_key == _last_page_title_key:
                            page_title = ""
                        elif _page_title_key:
                            _last_page_title_key = _page_title_key
            ...
            # 그룹화 모드(=계층 목차) 활성: 한 권에 화가 2개 이상 + flat_toc 아님
            _vol_chap_grouping = (
                not self.flat_toc
                and any(len(_g) >= 2 for _g in _vc_groups.values())
            )
            # flat 모드 + 권+화 패턴: epub당 1엔트리, 라벨에 화번호 보강
            _vol_chap_flat = (
                self.flat_toc
            ...
            if self.add_toc and toc_entries:
                ...
                if _vol_chap_grouping:
                    ...
                    for _ei_h, _lbl_h, _fh_h in _vc_ungrouped:
                        toc_html += (
                            f'<li><a href="{hm.escape(_fh_h)}">'
                            f'{hm.escape(_lbl_h)}</a></li>\n')
                    # 권 그룹
                    for _vn_h in sorted(_vc_groups.keys()):
                        ...
                        toc_html += (
                            f'<li><a href="{hm.escape(_entries_h[0][2])}">'
                            f'{hm.escape(_vlbl_h)}</a><ul>\n')
                        for _ei_c, _lbl_c, _fh_c, _cn_c in _entries_h:
                            toc_html += (
                                f'<li><a href="{hm.escape(_fh_c)}">'
                                f'{_cn_c}화</a></li>\n')
                        toc_html += '</ul></li>\n'
                elif _vol_chap_flat:
                    ...
                    for _ei_f, _lbl_f, _fh_f in toc_entries:
                        toc_html += (
                            f'<li><a href="{hm.escape(_fh_f)}">'
                            f'{hm.escape(_flat_lbl_for(_ei_f, _lbl_f))}</a></li>\n')
                else:
                    for _ei_n, lbl, opf_href in toc_entries:
                        ...
                        if _first_iid_h and _first_iid_h in _sub_navs and len(_sub_navs[_first_iid_h]) >= 2:
                            ...
                            for _si_h, (_anchor_h, _stitle_h) in enumerate(_subs_h):
                                ...
                                toc_html += (
                                    f'<li><a href="{hm.escape(_suri_h)}">'
                                    f'{hm.escape(_stitle_h)}</a></li>\n')
                        else:
                            toc_html += (
                                f'<li><a href="{hm.escape(opf_href)}">'
                                f'{hm.escape(lbl)}</a></li>\n')
            ...
            for iid, (href, mt) in all_manifest.items():
                ...
                cover_prop = ' properties="cover-image"' if sid == 'cover-image' else ''
                opf_lines.append(
                    f'    <item id="{sid}" href="{href}" media-type="{mt}"{cover_prop}/>')
            ...
            ni = 1
            # 표지 navPoint
            if all_spine and all_spine[0][0] == "cover-html":
                ncx_lines += [
                    f'        <navPoint id="navPoint-{ni}" playOrder="{ni}">',
                    f'            <navLabel><text>표지</text></navLabel>',
                    f'            <content src="{all_spine[0][1]}"/>',
                    f'        </navPoint>',
                ]
                ni += 1
                ...
                for _toc_epub_idx, lbl, first_href in toc_entries:
                    ...
                    if multi_vol:
                        ...
                        for iid, href, ptitle in book_pages:
                            ...
                            if _any_ptitle and _is_volume_only_toc_label(title, self.title):
                                continue
                            ncx_lines += [
                                f'        <navPoint id="book{book_idx:03d}_{iid}" playOrder="{ni}">',
            ...
            # ── container.xml ────────────────────────────────
            Path(meta_dir, "container.xml").write_text(
                '<?xml version="1.0" encoding="utf-8"?>'
            ...
            # ── mimetype ─────────────────────────────────────
            Path(out_dir, "mimetype").write_text(
                "application/epub+zip", encoding="utf-8")
            ...
            with zipfile.ZipFile(self.output_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                # mimetype은 ZIP_STORED
                mime_arcname = "mimetype"
                mime_src = os.path.join(out_dir, "mimetype")
                if _ts:
                    info = zipfile.ZipInfo(mime_arcname, date_time=_ts)
                    info.compress_type = zipfile.ZIP_STORED
                    with open(mime_src, 'rb') as _f:
                        zf.writestr(info, _f.read())
                else:
                ...
                for root, _, files in os.walk(out_dir):
                    for f in files:
                        ...
                        arcname = os.path.relpath(ap, out_dir)
                        if _ts:
    ...
    def _find_opf(self, base):
        c = Path(base, "META-INF", "container.xml")
        if not c.exists():
        ...
        return os.path.join(base, m.group(1).replace("/", os.sep))

    def _parse_opf(self, opf_path):
        content = Path(opf_path).read_text(encoding="utf-8", errors="replace")
        opf_dir = os.path.dirname(opf_path)
        manifest = {}
        # self-closing(/>) 과 non-self-closing(>) 둘 다 잡기
        _item_pat = re.compile(
            r'<item\s([^>]*?)\s*/?>', re.IGNORECASE)
        for m in _item_pat.finditer(content):
            attrs = m.group(1)
            mid  = re.search(r'\bid=["\']([^"\']+)["\']', attrs)
            href = re.search(r'\bhref=["\']([^"\']+)["\']', attrs)
            if mid and href:
                manifest[mid.group(1)] = href.group(1)
        spine = re.findall(r'<itemref\s[^>]*idref=["\']([^"\']+)["\']', content)
    ...
    def _parse_ncx(self, opf_dir):
        ...
        # NCX 파일 후보 경로
        candidates = [
            os.path.join(opf_dir, "toc.ncx"),
            os.path.join(opf_dir, "OEBPS", "toc.ncx"),
        ]
        ncx_path = next((p for p in candidates if os.path.exists(p)), None)
        if not ncx_path:
            # opf_dir 하위에서 재검색
            for root, _, files in os.walk(opf_dir):
                for f in files:
                    if f.lower() == "toc.ncx":
        ...
        try:
            raw = Path(ncx_path).read_text(encoding="utf-8", errors="replace")
            import html as _html
            # docTitle 추출 (단일화 epub의 완전한 제목)
            dt = re.search(r'<docTitle[^>]*>.*?<text[^>]*>(.*?)</text>', raw,
                           re.IGNORECASE | re.DOTALL)
            if dt:
                doc_title = _CDATA_PAT.sub(r'\1', re.sub(r'\s+', ' ', _html.unescape(dt.group(1)))).strip()
            # navPoint 블록별 파싱
            nav_pat = re.compile(
                r'<navPoint[^>]*>.*?<navLabel[^>]*>\s*<text[^>]*>(.*?)</text>.*?'
                r'<content[^>]*src=["\']([^"\'#]+)',
                re.IGNORECASE | re.DOTALL)
            for m in nav_pat.finditer(raw):
                label = _CDATA_PAT.sub(r'\1', re.sub(r'\s+', ' ', _html.unescape(m.group(1)))).strip()
                href  = m.group(2).strip()
                fname = Path(href.replace('\\', '/')).name.lower()
                if label and fname:
                    ncx_labels[fname] = label
        except Exception:
            pass
        return ncx_labels, doc_title
...
class TxtEpubWorker(QThread):
    ...
    def run(self):
        ...
        for idx, job in enumerate(self.jobs):
            normalized = None
            try:
                if _core_normalize_txt_epub_job and _core_write_txt_epub_job:
                    normalized = _core_normalize_txt_epub_job(
                        job,
                    ...
                    self.log_signal.emit(
                        f"📖 {Path(txt_path).name} → 변환 중... ({len(normalized.chapters)} 챕터)",
                        "info")
                    result = _core_write_txt_epub_job(normalized, self.output_dir)
                    self.log_signal.emit(
                        f"   ✅ 저장: {Path(result.output_path).name}  ({result.byte_count:,} bytes)", "ok")
                else:
                    # jobs 항목은 dict 또는 4-tuple (하위호환)
                    if isinstance(job, dict):
                        txt_path = job['path']
                        title    = job['title']
                        author   = job['author']
                        chapters = job['chapters']
                        # 파일별 표지가 있으면 우선, 없으면 공통 표지 폴백
                        _cv      = job.get('cover_data') or self.cover_data
                        _cv_ext  = job.get('cover_ext', '.jpg') if job.get('cover_data') else self.cover_ext
                    else:
                        txt_path, title, author, chapters = job
                        _cv     = self.cover_data
                        _cv_ext = self.cover_ext
                    self.log_signal.emit(
                        f"📖 {Path(txt_path).name} → 변환 중... ({len(chapters)} 챕터)",
                        "info")
                    data = build_txt_epub(chapters, title, author, _cv, _cv_ext)
                    safe_title = re.sub(r'[\\/:*?"<>|]', '_', title).strip() or "untitled"
                    _author_clean = (author or '').strip()
                    if _author_clean and _author_clean != "미상":
                        safe_author = re.sub(r'[\\/:*?"<>|\[\]]', '_', _author_clean).strip()
                        if not re.match(r'^\s*\[[^\]]+\]', title):
                            safe_name = f"[{safe_author}] {safe_title}"
                        else:
                            safe_name = safe_title
                    else:
                        safe_name = safe_title
                    out_path = Path(self.output_dir) / f"{safe_name}.epub"
                    i = 1
                    while out_path.exists():
                        out_path = Path(self.output_dir) / f"{safe_name} ({i}).epub"
                        i += 1
                    out_path.write_bytes(data)
                    self.log_signal.emit(
                        f"   ✅ 저장: {out_path.name}  ({len(data):,} bytes)", "ok")
                ok += 1
                self.succeeded_paths.append(str(txt_path))
            except Exception as ex:
...
def _html_to_plain_text(raw_html: str) -> str:
    s = re.sub(r'(?is)<(script|style)\b[^>]*>.*?</\1>', ' ', raw_html)
    s = re.sub(r'(?i)<br\s*/?>', '\n', s)
    s = re.sub(r'(?i)</(p|div|li|tr|td|th|h[1-6])\s*>', '\n', s)
    s = re.sub(r'(?s)<[^>]+>', ' ', s)
...
def _cleanup_txt_text(text: str) -> str:
    s = (text or "").replace('\r\n', '\n').replace('\r', '\n')
    # EPUB 내부 구조 라벨이 독립 라인으로 새어 나오는 경우 제거.
...
def _apply_txt_paragraph_indent(text: str) -> str:
    """TXT 출력용 문단 들여쓰기. 빈 줄은 유지하고 이미 들여쓴 줄은 건드리지 않는다."""
    out_lines = []
...
def extract_epub_text_sections(
    ...
    with zipfile.ZipFile(io.BytesIO(epub_bytes), 'r') as zf:
        ...
        opf_path = None
        try:
            container = _decode_markup_bytes(zf.read('META-INF/container.xml'))
        ...
        if opf_path and opf_path in names:
            opf_raw = _decode_markup_bytes(zf.read(opf_path))
            opf_dir = posixpath.dirname(opf_path)
            manifest: dict[str, str] = {}

            for im in re.finditer(r'<item\b[^>]*>', opf_raw, re.IGNORECASE):
                tag = im.group(0)
                item_id = _xml_attr(tag, 'id')
                href = _xml_attr(tag, 'href')
                media_type = _xml_attr(tag, 'media-type').lower()
            ...
            for sm in re.finditer(r'<itemref\b[^>]*>', opf_raw, re.IGNORECASE):
                item_id = _xml_attr(sm.group(0), 'idref')
        ...
        for rel in ordered_files:
            ...
            try:
                ...
                raw = _decode_markup_bytes(raw_bytes)
            except Exception:
                continue
            if remove_skip_pages:
                try:
                    reason = is_skip_page(rel, raw_bytes)
                except Exception:
                    reason = None
            ...
            if cleanup_text:
                text = _cleanup_txt_text(text)
            if text:
                sections.append((Path(rel).name, text))
        return sections
    ...
    def run(self):
        ...
        if self.combine_output:
            try:
                ...
                for idx, src_path in enumerate(sorted_jobs):
                    src_name = Path(src_path).name
                    src_ext = Path(src_path).suffix.lower()
                    if src_ext == ".txt":
                        self.log_signal.emit(f"🧹 {src_name} 정리 중...", "info")
                        raw = Path(src_path).read_bytes()
                        out_text = _decode_txt_bytes(raw)
                        if self.cleanup_text:
                            out_text = _cleanup_txt_text(out_text)
                        if self.indent_paragraphs:
                            out_text = _apply_txt_paragraph_indent(out_text)
                        ...
                        for _sec_name, sec_text in sections:
                            if self.indent_paragraphs:
                                sec_text = _apply_txt_paragraph_indent(sec_text)
                            lines.append(sec_text)
                            lines.append("")
                        out_text = "\n".join(lines).strip()
                    if out_text:
                        parts.append(Path(src_path).stem.strip())
                        parts.append("")
                        parts.append(out_text)
                        parts.append("")
                    self.progress_signal.emit(int((idx + 1) / max(total, 1) * 90))

                combined_text = "\n".join(parts).strip() + "\n"
                stem = re.sub(r'[\\/:*?"<>|]', '_', self.combined_stem).strip() or "merged"
                out_path = Path(self.output_dir) / f"{stem}.txt"
                n = 1
                while out_path.exists():
                    out_path = Path(self.output_dir) / f"{stem} ({n}).txt"
                    n += 1
                out_path.write_text(combined_text, encoding='utf-8')
                self.log_signal.emit(f"   ✅ 저장: {out_path.name}", "ok")
                ok += 1
                self.progress_signal.emit(100)
            except Exception as ex:
        ...
        for idx, src_path in enumerate(self.jobs):
            try:
                ...
                stem = re.sub(r'[\\/:*?"<>|]', '_', Path(src_path).stem).strip() or "output"
                out_path = Path(self.output_dir) / f"{stem}.txt"
                n = 1
                while out_path.exists():
                    out_path = Path(self.output_dir) / f"{stem} ({n}).txt"
...
class EPUBMergerGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Epub Binder v4.3.4")
    ...
    def _volume_range(filenames: list) -> str:
        def _extra_labels() -> list[str]:
            ...
            for _fn in filenames:
                ...
                if re.search(r'특별\s*외전|특\s*외', _stem):
                    if '특외' not in labels:
                        labels.append('특외')
                elif re.search(r'외전|번외|특전', _stem):
                    if '외전' not in labels:
                        labels.append('외전')
            return labels
    ...
    def _open_cover_picker(self):
        """현재 1권에서 표지 후보를 추출해 다이얼로그를 연다.
        파일이 없으면 알림. 후보 0개면 알림."""
        if not self._files:
    ...
    def _maybe_prompt_cover_choice(self, first_epub_path: str) -> bool:
        """1권에 표지 후보가 2개 이상이고 사용자 지정 표지가 없으면 다이얼로그를 띄운다.
        ...
        self._log(f"⚠ 1권에 표지 후보 {len(cands)}개 감지: {names}", "warn")
        ...
        dlg = CoverPickerDialog(
            ...
            epub_bytes=eb,
            file_label=Path(first_epub_path).name)
        if dlg.exec() == QDialog.DialogCode.Accepted:
                        ...
                        for sid in spine_ids:
                            ...
                            if hasattr(self, '_page_title_overrides') and override_key in self._page_title_overrides:
                                ...
                                if ncx_labels_dlg:
                                    ...
                                    elif False and ptitle and not re.search(r'\d+\s*[화권]', ptitle):
                                        # 화번호 없는 navLabel: HTML 우선 시도
                                        html_title = extract_chapter_title(data)
                                        if html_title and re.search(r'\d+\s*[화권]', html_title):
                                            ptitle = html_title
                                        elif html_title and re.search(r'\d+', html_title):
                                            # 화/권 없어도 숫자가 있으면 실제 챕터 제목으로 사용
                                            # 예) "Chapter 1", "Chapter 6" 등 단행본 영어 챕터 제목
                                            ptitle = html_title
                                        elif html_title and (
                                                _is_chapterish_title(html_title)
                                                or _SUBNAV_HEAD_PAT.match(_normalize_title(html_title))):
                                            # 숫자가 없어도 프롤로그/에필로그/외전 등은 챕터 제목으로 인정
                                            ptitle = html_title
                                        elif ncx_doc_title_dlg:
                                            chap_m = re.search(r'(\d+)\s*화', ncx_doc_title_dlg)
                                            if chap_m:
                                                ptitle = f'{chap_m.group(1)}화 {ptitle}'
                                else:
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    def _build_dedupe_tab(self, root):
        ...
        b_all = mk_btn("안전 후보 선택", "gray")
        b_all.setToolTip("신뢰도 높은 정리 후보만 선택합니다. 확인 필요 항목은 수동으로 체크해 주세요.")
...
class NaverSeriesFetchThread(QThread):
    ...
    def run(self):
        ...
        try:
            no = self.product_no.strip()

            # ── Step 1: detail 페이지 로드 (novel → comic 순서로 시도) ─
            detail_html = None
            category    = None
            detail_url  = None
            for cat in ('novel', 'comic'):
                url = f"https://series.naver.com/{cat}/detail.series?productNo={no}"
                self.progress.emit(f"[1] detail 페이지 로드 중... ({cat}, productNo={no})")
                try:
                    detail_html = _get(url)
                    # 존재하는 작품이면 og:title이 있음
                    if re.search(r'<meta[^>]+property=["\']og:title["\']', detail_html):
                        category = cat
                        detail_url = url
                        self.progress.emit(f"✓ 카테고리: {cat}")
                        break
                    detail_html = None
                except Exception:
                    continue
            ...
            if not detail_html:
                self.failed.emit(
                    f"productNo={no} 에 해당하는 작품을 찾지 못했습니다.\n"
                    "URL/productNo가 올바른지 확인해주세요.")
                return

            # ── Step 2: originalProductId 추출 ──────────────────────
            orig_id = None
            for pat in [
                r'["\']originalProductId["\']\s*:\s*["\']?(\d+)["\']?',
                r'"originalProductId"\s*:\s*(\d+)',
                r"'originalProductId'\s*:\s*(\d+)",
                r'originalProductId\s*[:=]\s*(\d+)',
                r'data-original-product-id=["\'](\d+)["\']',
            ]:
            ...
            if orig_id:
                self.progress.emit(f"✓ originalProductId: {orig_id}")
            else:
                self.progress.emit("⚠ originalProductId 미발견 — CDN 시도 생략")

            img_url   = None
            img_data  = None
            img_ctype = 'image/jpeg'

            # ── pstatic URL 수집 헬퍼 (src / data-src / data-original 모두) ──
            def _find_pstatic(html, keyword=None):
                """html 에서 pstatic.net 이미지 URL을 src/data-src/data-original 순으로 수집."""
                urls = re.findall(
                    r'(?:src|data-src|data-original|data-lazy)=["\']'
                    r'(https://[^"\']*\.pstatic\.net/[^"\']+)'
                    r'["\']',
                    html)
                if keyword:
                    priority = [u for u in urls if keyword in u]
                    return priority if priority else urls
                return urls

            def _abs_url(url):
                return urllib.parse.urljoin("https://series.naver.com/", html_lib.unescape(url))
            ...
            def _with_param(url, key, value):
                ...
                return urllib.parse.urlunsplit((
                    ...
                    urllib.parse.urlencode(new_query), p.fragment))
            ...
            def _with_page(url, page):
                ...
                for k, v in query:
                    if k == 'page':
                        new_query.append((k, str(page)))
                        found = True
                    else:
                        new_query.append((k, v))
                if not found:
                    new_query.append(('page', str(page)))
                return urllib.parse.urlunsplit((
                    p.scheme, p.netloc, p.path,
                    urllib.parse.urlencode(new_query), p.fragment))

            def _get_json(url, referer=None):
                return json.loads(_get(url, referer=referer))

            def _volume_list_url():
                base_url = None
                for pat in [
                    ...
                    m = re.search(pat, detail_html)
                    if m:
                        base_url = _abs_url(m.group(1))
                        break
                if not base_url:
                    total = "9999"
                    m = re.search(r'totalCount\s*[:=]\s*["\']?(\d+)', detail_html)
                    if m:
                        total = m.group(1)
                    base_url = (f"https://series.naver.com/{category}/volumeList.series"
                                f"?productNo={no}&totalCount={total}")
                # 항상 오름차순 강제 — 1화가 1페이지에 오도록
                return _with_param(base_url, 'sortOrder', 'ASC')

            def _row_text(row):
                parts = []
                for key in ('volumnNameText', 'volumeName', 'subProductName',
                            'productName', 'expansionProductName'):
                    val = row.get(key)
                    if val:
                        parts.append(str(val))
                return " / ".join(parts)

            def _row_episode_no(row):
                for key in ('volumeNo', 'seriesNo'):
                    try:
                        ...
                        pass
                text = _row_text(row)
                for pat in (r'(?:제\s*)?(\d+)\s*화', r'\b(\d+)\s*회\b'):
                    m = re.search(pat, text)
                    if m:
                        return int(m.group(1))
                return 999999999

            def _row_detail_url(row):
                for key in ('detailPCPageUrlByNstoreKey', 'detailPageUrlByNstoreKey'):
                    val = row.get(key)
                    if val:
                        return _abs_url(str(val))
                pno = row.get('productNo')
                if pno:
                    return f"https://series.naver.com/{category}/volumeDetail.series?productNo={pno}"
                return None

            def _load_volume_rows():
                # sortOrder=ASC 강제로 1화가 1페이지에 위치 → 1페이지만 로드
                base = _volume_list_url()
                page_url = _with_page(base, 1)
                obj = _get_json(page_url, referer=detail_url)
                data = obj.get('resultData', []) if isinstance(obj, dict) else []
                if not data:
                    self.progress.emit("  ⚠ 회차 목록이 비어 있습니다.")
                    return []
                rows, seen = [], set()
                for row in data:
                    ...
                    if key:
                        seen.add(key)
                    rows.append(row)
                self.progress.emit(f"  ✓ 1페이지: {len(rows)}개 수집")
                return rows

            def _normalize_img_url(url):
                url = html_lib.unescape(str(url)).replace('\\/', '/').strip()
                url = url.strip('"\'')
                if not url.startswith(('http://', 'https://')):
                    return None
                p = urllib.parse.urlsplit(url)
                if 'pstatic.net' not in p.netloc.lower():
                    return None
                if not re.search(r'\.(?:jpe?g|png|webp|gif)$', p.path.lower()):
                    return None
                query = p.query
                # Naver thumbnail URLs often append ?type=m260/m600x314.
                # Removing that query requests the original-size cover.
                if 'type=' in query:
                    query = ''
                return urllib.parse.urlunsplit((p.scheme, p.netloc, p.path, query, ''))

            def _extract_cover_urls(html, preferred_ids=()):
                raw_urls = []
                patterns = [
                    r'<meta[^>]+property=["\']og:image["\'][^>]+content=["\']([^"\']+)',
                    r'<meta[^>]+content=["\']([^"\']+)[^>]+property=["\']og:image["\']',
                    r'(?:src|data-src|data-original|data-lazy)=["\']([^"\']*pstatic\.net[^"\']+)["\']',
                    r'https://[^\s"\'<>\]\\]+\.pstatic\.net/[^\s"\'<>\]\\]+',
                ]
                for pat in patterns:
                    raw_urls.extend(re.findall(pat, html))
                ...
                preferred_ids = [str(x) for x in preferred_ids if x]

                def _score(url):
                    lower = url.lower()
                    score = 0
                    if any(pid in url for pid in preferred_ids):
                        score -= 120
                    if 'comicthumb' in lower:
                        score -= 40
                    if 'thumbnailbig' in lower:
                        score -= 30
                    if re.search(r'favicon|/static/|noimg|loading|thumb/no|thumb/19|gnb_', lower):
                        score += 500
                    if 'type=m101' in lower:
                        score += 60
                    return score
            ...
            def _try_download_candidates(candidates, label, referer=None, min_bytes=20_000):
                for cand in candidates:
                    try:
                        data, ctype = _get_bytes(cand, referer=referer)
                    except Exception as ex:
                        self.progress.emit(f"  ⚠ 다운로드 실패: {ex}")
                        continue
                    if not str(ctype).lower().startswith('image/'):
                        self.progress.emit(f"  ⚠ 이미지가 아님: {cand}")
                        continue
                    if len(data) < min_bytes:
                        self.progress.emit(f"  ⚠ 너무 작은 이미지 제외: {len(data)//1024:,} KB")
                        continue
                    self.progress.emit(f"  ✓ {label}: {len(data)//1024:,} KB")
                    return cand, data, ctype
                return None, None, None

            # URL 패턴 구분:
            #  에피소드(155KB): .../novel/{origId}/{episodeId}/thumbnail/thumbnailbig.jpg  ← 숫자 2개
            #  시리즈(133KB):  .../novel/{origId}/thumbnail/thumbnailbig.jpg              ← 숫자 1개
            def _scan_episode_urls(html):
                """HTML 어디서든(속성·JS·JSON) 에피소드급 URL 수집 — /숫자/숫자/thumbnail/ 패턴."""
                return re.findall(
                    r'https://[^\s"\'<>\]\\]+\.pstatic\.net'
                    r'/[^\s"\'<>\]\\]+/\d+/\d+/thumbnail/thumbnailbig\.[a-z]+',
                    html)

            def _scan_series_urls(html):
                """HTML 어디서든 시리즈급 URL 수집 — /숫자/thumbnail/ 패턴."""
                return re.findall(
                    r'https://[^\s"\'<>\]\\]+\.pstatic\.net'
                    r'/[^\s"\'<>\]\\]+/\d+/thumbnail/thumbnailbig\.[a-z]+',
                    html)

            # ── Step 3-A: volumeList 전체 수집 → 1화 상세 표지 우선 ─
            self.progress.emit("[3-A] 회차 목록에서 1화 표지 탐색...")
            try:
                rows = _load_volume_rows()
                if rows:
                    self.progress.emit(f"✓ 총 {len(rows)}개 회차 발견")
                    target = min(rows, key=lambda r: (
                        _row_episode_no(r),
                        int(r.get('productNo') or 0) if str(r.get('productNo') or '').isdigit() else 0))
                    ep_no = _row_episode_no(target)
                    ep_name = target.get('volumnNameText') or (f"{ep_no}화" if ep_no < 999999999 else "선택 회차")
                    detail_ep_url = _row_detail_url(target)
                    self.progress.emit(
                        f"  → 선택 회차: {ep_name} (productNo={target.get('productNo', '')})")
                    if detail_ep_url:
                        ep_html = _get(detail_ep_url, referer=detail_url)
                        preferred = [
                        ...
                        candidates = _extract_cover_urls(ep_html, preferred)
                        img_url, img_data, img_ctype = _try_download_candidates(
                            candidates, f"{ep_name} 표지", referer=detail_ep_url, min_bytes=20_000)
                    else:
                        self.progress.emit("  ⚠ 선택 회차 상세 URL을 찾지 못했습니다.")
            except Exception as ex:
                self.progress.emit(f"  ⚠ volumeList 실패: {ex}")

            # ── Step 3-B: 작품 상세 페이지 원본 표지 폴백 ───────────
            if not img_url:
                self.progress.emit("[3-B] 작품 상세 페이지 원본 표지 탐색...")
                candidates = _extract_cover_urls(detail_html, [orig_id])
                img_url, img_data, img_ctype = _try_download_candidates(
                    candidates, "상세 페이지 표지", referer=detail_url, min_bytes=20_000)

            # ── Step 3-C: 기존 thumbnailbig 패턴 폴백 ───────────────
            if not img_url:
                self.progress.emit("[3-C] thumbnailbig 패턴으로 폴백 탐색...")
                ep_urls = [_normalize_img_url(u) for u in _scan_episode_urls(detail_html)]
                ep_urls = [u for u in ep_urls if u]
                if ep_urls:
                    img_url, img_data, img_ctype = _try_download_candidates(
                        ep_urls, "에피소드 표지(detail)", referer=detail_url, min_bytes=20_000)
                if not img_url:
                    ser_urls = [_normalize_img_url(u) for u in _scan_series_urls(detail_html)]
                    ser_urls = [u for u in ser_urls if u]
                    img_url, img_data, img_ctype = _try_download_candidates(
                        ser_urls, "시리즈 표지(detail)", referer=detail_url, min_bytes=8_000)

            # ── Step 3-D: originalProductId CDN 직접 구성 ───────────
            if not img_url and orig_id:
                self.progress.emit("[3-D] CDN URL 패턴으로 시도 중...")
                cdn_list = [
                    f"https://image-comic.pstatic.net/series/{orig_id}/thumbnail/thumbnailbig.jpg",
                    f"https://image-comic.pstatic.net/{category}/{orig_id}/thumbnail/thumbnailbig.jpg",
                    f"https://image-comic.pstatic.net/series/{orig_id}/thumbnail/thumbnail.jpg",
                ]
                for cdn_url in cdn_list:
                    self.progress.emit(f"  → {cdn_url}")
                    try:
                        data, ctype = _get_bytes(cdn_url)
                        if len(data) > 8_000:
                            img_url   = cdn_url
                            img_data  = data
                            img_ctype = ctype
                            self.progress.emit(f"  ✓ {len(data)//1024:,} KB")
                            break
                    except Exception:
                        continue

            # ── Step 3-E: og:image 최후 수단 ────────────────────────
            if not img_url:
                for pat in [
                    ...
                    m = re.search(pat, detail_html)
                    if m:
                        og_url = _normalize_img_url(m.group(1).strip())
                        if og_url:
                            img_url, img_data, img_ctype = _try_download_candidates(
                                [og_url], "og:image", referer=detail_url, min_bytes=8_000)
                        if img_url:
                            self.progress.emit(f"⚠ og:image 사용: {img_url}")
                        break

            if not img_url:
                self.failed.emit("표지 이미지 URL을 찾지 못했습니다.")
                return

            # ── Step 4: 이미지 다운로드 ─────────────────────────────
            if img_data is None:
                self.progress.emit(f"[4] 이미지 다운로드 중...")
                img_data, img_ctype = _get_bytes(img_url)

            # ── Step 5: 제목 / 작가 추출 ────────────────────────────
            fetched_title  = ""
            fetched_author = ""
            def _plain_meta(raw: str) -> str:
                raw = re.sub(r'<br\s*/?>', ' ', raw, flags=re.IGNORECASE)
                raw = re.sub(r'<[^>]+>', '', raw)
            ...
            def _clean_author(raw: str) -> str:
                val = _plain_meta(raw)
                val = re.sub(r'^(?:글|작가|저자|원작)\s*[:：|｜]?\s*', '', val).strip()
                val = re.sub(r'\s*(?:그림|출판사|전체\s*이용가).*$','', val).strip()
                if re.search(r'^\d|TOP|무료|시리즈|출판사|현판|로맨스|판타지', val, re.IGNORECASE):
                    return ''
                return val if 1 < len(val) <= 30 else ''
            ...
            for pat in [
                ...
                m = re.search(pat, detail_html)
                if m:
                    fetched_title = html_lib.unescape(m.group(1)).strip()
                    break

            # 작품 상세 영역의 "글 <a>작가</a>"를 최우선으로 사용한다.
            # 페이지 하단 TOP10 작가(class=author)가 먼저 잡히는 오탐을 막기 위해
            # 인기/추천 영역 앞쪽만 broad 패턴에 넘긴다.
            detail_main_html = detail_html
            _info_m = re.search(
                r'<ul[^>]+class=["\'][^"\']*end_info[^"\']*["\']',
                detail_html, re.IGNORECASE)
            if _info_m:
                # 작품 메타 영역만 사용하면 하단/사이드 TOP10 작가 오탐을 피할 수 있다.
                detail_main_html = detail_html[_info_m.start():_info_m.start() + 5000]

            for pat in [
                r'(?:글|저자|작가)\s*<a[^>]*>(.*?)</a>',
                r'<span[^>]*>\s*(?:글|저자|작가)\s*</span>\s*<a[^>]*>(.*?)</a>',
                r'<dt[^>]*>\s*(?:글|저자|작가)\s*</dt>\s*<dd[^>]*>(.*?)</dd>',
                r'<li[^>]*>\s*(?:글|저자|작가)\s*<a[^>]*>(.*?)</a>',
            ]:
                m = re.search(pat, detail_main_html, re.DOTALL | re.IGNORECASE)
                if m:
                    fetched_author = _clean_author(m.group(1))
                    if fetched_author:
                        break

            for pat in [
                r'<dd[^>]+class=["\'][^"\']*author[^"\']*["\'][^>]*>(.*?)</dd>',
                r'"authorName"\s*:\s*"([^"]+)"',
                r"'authorName'\s*:\s*'([^']+)'",
                r'<span[^>]+class=["\'][^"\']*author[^"\']*["\'][^>]*>([^<]+)</span>',
                r'<em[^>]+class=["\'][^"\']*author[^"\']*["\'][^>]*>([^<]+)</em>',
            ]:
                if fetched_author:
                    break
                m = re.search(pat, detail_main_html, re.DOTALL | re.IGNORECASE)
            ...
            kb = len(img_data) // 1024
            self.progress.emit(f"✅ 완료: {kb:,} KB ({img_url.split('/')[-1]})"
                               + (f" | 작가: {fetched_author}" if fetched_author else ""))
            self.finished.emit(img_data, _ext_from(img_url, img_ctype),
                               fetched_title, fetched_author)
            ...
            if e.code == 403:
                ...
                self.failed.emit(f"HTTP 오류 {e.code}: {e.reason}")
        except Exception as ex:
            self.failed.emit(f"오류: {ex}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    _EXPIRE = datetime.date(2026, 7, 31)
    if datetime.date.today() > _EXPIRE:
        QMessageBox.critical(None, "사용 기간 만료",
            f"이 버전의 사용 기간이 만료되었습니다.\n\n만료일: {_EXPIRE.strftime('%Y년 %m월 %d일')}")
        sys.exit(0)
    gui = EPUBMergerGUI()
    gui.show()
    sys.exit(app.exec())
