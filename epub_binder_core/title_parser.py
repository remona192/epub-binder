...
def _strip_metadata_title_label(text: str) -> str:
    return clean_text(re.sub(r"^\s*(?:제목|title)\s*[:：]\s*", "", text or "", flags=re.IGNORECASE))


def _filename_episode_range_marker(value: str) -> str:
    text = clean_text(value)
    for match in re.finditer(
        r"(?<!\d)(?P<start>\d{1,5})\s*[-~]\s*(?P<end>\d{1,5})(?:\s*(?:화|회|장))?(?:\s*[-–—_ ]+\s*(?:完|완|완결))?",
        text,
        re.IGNORECASE,
    ):
...
def _split_trailing_side_story(text: str) -> tuple[str, str]:
    value = clean_text(text)
    match = re.search(r"\s*((?:특별\s*)?외전(?:\s*포함)?|번외|특전)\s*$", value, re.IGNORECASE)
    if not match:
        return value, ""
    return clean_text(value[:match.start()]), clean_text(match.group(1))
...
def parse_epub_name(
    ...
    _meta_title_clean = _strip_metadata_title_label(meta_title)
    ...
    _filename_has_episode = bool(
        _EPISODE_RE.search(_filename_body_clean)
        or _filename_episode_range_marker(_filename_body_clean)
    )

    if _filename_has_vol or _filename_has_episode:
        # 파일명에 N권/N화/회차범위 패턴 있음 → 파일명 body 그대로 사용 (시리즈명/화수 보존)
        body = _filename_body_clean
    else:
        heading = _best_heading(html_headings)
        meta_title_reliable = bool(
            _meta_title_clean
            and not _looks_like_chapter_heading(_meta_title_clean)
            and not _GENERIC_BOOK_HEADING_RE.match(_meta_title_clean)
        )
        ...
        if heading and _meta_title_clean and re.search(r"\d", _meta_title_clean):
            if not re.search(r"\d+\s*(?:\uAD8C|\uD654|\uC7A5|\uBD80|\uD3B8)\b", clean_text(heading)):
                heading = ""
        if heading and _meta_title_clean and _looks_like_chapter_heading(heading) and not _looks_like_chapter_heading(_meta_title_clean):
            heading = ""
        ...
        if meta_title_reliable and not (heading_has_volume_marker and filename_is_generic_section):
            source = clean_text(_meta_title_clean or body)
        else:
            source = clean_text(_filename_marker_source or heading or _meta_title_clean or body)
        if source:
